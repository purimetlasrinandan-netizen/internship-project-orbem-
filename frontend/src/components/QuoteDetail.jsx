...
========================================
    ...
    AIR FREIGHT QUOTATION SUMMARY
========================================
Quote Reference : ${q.id}
Created Date    : ${new Date(q.created_at).toLocaleString()}
Client Company  : ${q.customer_company}
Shipper Contact : ${q.customer_name} (${q.customer_email})
----------------------------------------
Route Info:
  Origin Airport      : ${q.origin}
  Destination Airport : ${q.destination}
  Urgency Level       : ${q.urgency}
----------------------------------------
Cargo Info:
  Cargo Type      : ${q.cargo_type}
  Package Count   : ${q.package_count} units
  Actual Weight   : ${q.actual_weight} kg
  Dimensions      : ${q.length} x ${q.width} x ${q.height} cm per unit
  Volumetric Wt   : ${q.volumetric_weight} kg
  Chargeable Wt   : ${q.chargeable_weight} kg
----------------------------------------
Costing breakdown (USD):
  Base Freight Cost : $${q.base_price.toFixed(2)}
  Fuel Surcharge    : $${q.fuel_surcharge.toFixed(2)}
  Security Charge   : $${q.security_charge.toFixed(2)}
  Urgency Charge    : $${q.urgency_charge.toFixed(2)}
  --------------------------------------
  Total Quote Price : $${q.total_price.toFixed(2)}
----------------------------------------
Status  : ${q.status}
Owner   : ${q.owner}
Remarks : ${q.remarks || 'None'}
========================================
    Generated by ORBEM System. Thank you.
`;
    const element = document.createElement("a");
    const file = new Blob([docContent], {type: 'text/plain'});
    element.href = URL.createObjectURL(file);
    element.download = `Quotation_${q.id}.txt`;
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button className="btn btn-secondary" onClick={handleExportText}>
            <Download size={16} /> Export PDF/TXT
          </button>
